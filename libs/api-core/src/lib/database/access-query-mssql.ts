...
export class QueryColumnsMsSql {
  constructor(
  ...
  addJsonEntityColumn(entityName: string) {
    this.columns.push(
      this.accessContext.knex.raw(
        `${this.baseTable}.${IModelFields.json_document}.${entityName} as ${entityName}`
      )
    );
  }
  ...
  addJsonEntityColumnForJoinTable(tableName: string, entityName: string) {
    this.columns.push(
      this.accessContext.knex.raw(
        `JSON_QUERY(${tableName}.${IModelFields.json_document},'$.${entityName}') as ${entityName}`
      )
    );
  }
...
export class AccessQueryMsSql implements IAccessQuery {
  private joins: { [tableName: string]: {} } = {};
  private leftJoins: { [tableName: string]: {} } = {};
  private columns = [];
  private query: QueryBuilder;
  private baseTable: string;
  public Columns: QueryColumnsMsSql;
  ...
  constructor(
    ...
    this.Columns = new QueryColumnsMsSql(
      this.baseTable,
      this.columns,
      this.accessContext
    );
  ...
  addWhereJson(entityName: string, columnName: string, value: string | number) {
    this.query = this.query.whereRaw(
      `JSON_VALUE(${this.baseTable}.${IModelFields.json_document},'$.${entityName}.${columnName}') = ?`,
      value
    );
  }
  ...
  addWhereJsonPath(
    ...
    this.query = this.query.whereRaw(
      `JSON_VALUE(${this.baseTable}.${
        IModelFields.json_document
      },'$.${this.translateJsonPath(path)}')${comparison} ? `,
      value
    );
  }
  ...
  addWhereJsonPathLike(path: string[], value: string | number | boolean) {
    this.query = this.query.whereRaw(
      `lower(JSON_VALUE(${this.baseTable}.${
        IModelFields.json_document
      },'$.${this.translateJsonPath(path)}')) like lower(?) `,
      value
    );
  }
  ...
  addWhereJsonLike(entityName: string, columnName: string, value: string) {
    this.query = this.query.whereRaw(
      `lower(JSON_VALUE(${this.baseTable}.${IModelFields.json_document},'$.${entityName}.${columnName}')) like lower(?)`,
      value
    );
  }
  ...
  addWhereJsonOrList(fieldList: JsonFieldCompare[]) {
    ...
    if (fieldList.length > 0) {
      ...
      this.query = this.query.andWhere(function () {
        this.whereRaw(
          `JSON_VALUE(${baseTable}.${IModelFields.json_document},'$.${jsonPath}') like ? `,
          fieldList[0].value
        );
  ...
  addWhereJsonIn(
    ...
    raw += `JSON_VALUE(${tableName}.json_document,'$`;
    while (path.length > 1) {
      raw += `.${path.pop()}`;
    }
  ...
  addWhereJsonAttributeLike(columnName: string, value: string) {
    this.query = this.query.whereRaw(
      `JSON_VALUE(lower(${this.baseTable}.${IModelFields.json_document},'$.${columnName}') like lower(?)`,
      value
    );
  }
  ...
  addOrderByJsonAttribute(attributeName: string, direction: SortDirection) {
    this.query = this.query.orderByRaw(
      `JSON_VALUE("${this.baseTable}".${IModelFields.json_document},'$.${attributeName}') ${direction} `
    );
  }
  addOrderByJsonEntityAttribute(
    ...
    this.query = this.query.orderByRaw(
      `JSON_VALUE("${this.baseTable}".${IModelFields.json_document},'$.${entityName}.${attributeName}') ${direction} `
    );
  }
  ...
  addOrderByJsonEntityColumnForJoinTable(
    ...
    this.query = this.query.orderByRaw(
      `JSON_VALUE("${tableName}".${IModelFields.json_document},'$.${entityName}.${attributeName}') ${direction} `
    );
  }
